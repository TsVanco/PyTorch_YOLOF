...
def train():
    ...
    dataloader = build_dataloader(
        args, dataset, args.batch_size, CollateFunc())

    # build model & criterion
    model, criterion = build_model(
        args=args, cfg=cfg, device=device, num_classes=num_classes, trainable=True)
    ...
    lr_scheduler = torch.optim.lr_scheduler.MultiStepLR(
        optimizer=optimizer, milestones=lr_epoch)
    ...
    for epoch in range(max_epoch):
        ...
        for iter_i, (images, targets, masks) in enumerate(dataloader):
            ...
            if distributed_utils.is_main_process() and iter_i % 10 == 0:
                ...
                log += '[lr: {:.6f}][lr_bk: {:.6f}]'.format(
                    cur_lr_dict['lr'], cur_lr_dict['lr_bk'])
                # loss infor
                ...
                log += '[size: [{}, {}]]'.format(
                    cfg['train_min_size'], cfg['train_max_size'])
